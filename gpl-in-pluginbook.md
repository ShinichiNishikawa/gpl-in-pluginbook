...
GPLは、再配布の自由を認めています。その際、「無料でないといけない」という条件は付きません。つまり、テーマやプラグインを有償で販売することはライセンス違反ではありません。ただし、販売は配布に含まれますので、そのソフトウェアもGPL互換ライセンスであることが必須となり、購入者（ユーザー）によるあらゆる用途での利用、ソースコードの開示・改変やソフトウェアの再配布を制限することはできません。

また、誤解の多い点ですが、GPLはソフトウェアのソースコードを公開しないで利用することも可能です。GPLで配布されているソフトウェアを受け取り、改変・利用しているだけでは、そのソースを公開する必要はないからです。これは、WordPressを利用したウェブサービスやアプリケーションなどを制作・運用する場合にも当てはまります。
たとえば、あなたがクライアントのためにテーマやプラグインを制作した場合、クライアントに配布したファイルにGPL互換のライセンスを適用する必要はありますが、それらのソースコードを公開する「義務」はあなたにもクライアントにもありません。この場合は、唯一のユーザーであるクライアントだけが、あなたに対してソースコードの開示を要求できることになります。
ただ、そういったテーマやプラグインを公開してしまうと、そのユーザーとなり得る公衆に対してソースコードを開示する義務が生じてしまうので、気をつけてください。
...
次の節では、WordPressを使っている世界のビジネスにはどのようなものがあるのか、分類や事例を挙げながらご紹介していきます。